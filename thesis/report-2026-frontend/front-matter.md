...
This individual report covers the front end, analytics, testing and documentation. The front end is a React single-page application with three role-based portals (reportee, police, admin) in fourteen pages, built with four runtime packages. Seven analytics endpoints compute recovery rate, recovery time, detections and alert statistics from stored records, and a shared expression corrects a defect in which detection confidence stored on two scales (fraction and percentage) produced an average of about 30 instead of about 87. The system is supported by 39 automated tests, a README and a run script.

The application builds into a 263.6 kB JavaScript bundle (82.5 kB gzipped), and all 39 Python tests pass. A line-tracing measurement, made with a standard-library tracer because the usual tool could not be installed, showed that the tests run about 63.6% of the backend's lines, with user management (27%) and system routes (32%) least covered. A scripted browser session found that a wrong password gives no error message because the page reloads, and further checks found unhandled loading failures, non-proportional and gappy charts, form labels not associated with inputs, no front-end tests, and README instructions that no longer match the code, including a setting the backend ignores. None of these findings has been fixed in this project. No user evaluation was carried out.

**Keywords:** React; single-page application; analytics; software testing; code coverage
...
<span class="todo">[Write acknowledgements here: supervisor, group members (AI/vision, backend and security, real-time and integration), family, and anyone who helped collect plate images. The author must write this; it is personal.]</span>
